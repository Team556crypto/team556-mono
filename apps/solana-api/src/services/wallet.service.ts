...
export class SolanaWalletService {
   ...
   */
  async createTokenAccount(ownerOrKeypair: Keypair | PublicKey, mint: PublicKey, payerKeypair?: Keypair) {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Transaction confirmation timed out after ${timeout / 1000} seconds. Final status: ${JSON.stringify(status?.value?.confirmationStatus || 'unknown')}`
        )
      } catch (error: any) {
  ...
  async sendToken(senderKeypair: Keypair, recipientAddress: string, mint: string, amount: number) {
    try {
      ...
      instructions.push(
        this.createTransferInstruction(senderTokenAccount, recipientTokenAccount, senderKeypair.publicKey, amount)
      )
