...
import { setupProvider, getProgram, loadKeypair, MINT_ADDRESS, TOKEN_DECIMALS } from './common'
import fs from 'fs'
...
async function main() {
  ...
  for (let i = 0; i < vestingAccounts.length; i++) {
    ...
    vestingAccount.schedule.forEach((item, idx) => {
      log(
        `    [${idx}] ${item.releaseTime.toString()} (${new Date(item.releaseTime.toNumber() * 1000).toISOString()}) - ${item.amount.toString()} tokens (${item.amount.toNumber() / Math.pow(10, TOKEN_DECIMALS)} $TEAM)`
      )
    ...
    log(
      `  Total unlocked: ${totalUnlocked.toString()} tokens (${totalUnlocked.toNumber() / Math.pow(10, TOKEN_DECIMALS)} $TEAM)`
    )
    log(
      `  Already claimed: ${vestingAccount.claimedAmount.toString()} tokens (${vestingAccount.claimedAmount.toNumber() / Math.pow(10, TOKEN_DECIMALS)} $TEAM)`
    )
    log(
      `  Available to claim: ${claimableAmount.toString()} tokens (${claimableAmount.toNumber() / Math.pow(10, TOKEN_DECIMALS)} $TEAM)`
    )
    ...
    try {
      ...
      const beneficiaryTokenAccount = getAssociatedTokenAddressSync(MINT_ADDRESS, beneficiary)
  ...
  log(
    `Total tokens distributed: ${totalAmount.toString()} (${totalAmount.toNumber() / Math.pow(10, TOKEN_DECIMALS)} $TEAM)`
  )
