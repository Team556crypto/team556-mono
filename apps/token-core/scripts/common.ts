...
import { Team } from '../../../packages/target/types/team'
...
export function loadKeypair(keypairPath: string): Keypair {
  ...
  const loaded = Keypair.fromSecretKey(new Uint8Array(JSON.parse(fs.readFileSync(keypairPath).toString())))
...
if (!process.env.PROGRAM_ID) {
  console.warn("PROGRAM_ID not found in .env, relying on Anchor.toml. Ensure it's correct.")
}
