      ...
      <View style={styles.contentContainer}>
        ...
        <View style={styles.headerContainer}>
          ...
          <Text style={styles.subheading}>
            From setup to daily use, your journey with Team556 is straightforward, secure, and designed for an
        ...
        {(Platform.OS !== 'web' || Dimensions.get('window').width < 1024) && (
          <View style={styles.mobileContainer}>
            ...
            <View style={styles.stepSelector}>
              <View style={[styles.progressBar, { width: `${(activeStep / steps.length) * 100}%` }]} />
              ...
              {steps.map(step => (
                ...
                >
                  <Text style={[styles.stepButtonNumber, activeStep === step.id && styles.activeStepText]}>
                    {step.id.toString()}
                  </Text>
                  <Text style={[styles.stepButtonLabel, activeStep === step.id && styles.activeStepText]}>
                    {step.title.split(' ')[0]}
                  </Text>
                </Pressable>
              ))}
                  ...
                  <View key={`content-${step.id}`} style={styles.mobileStepContent}>
                    <View style={styles.mobileStepCard}>
                      <View style={styles.mobileStepHeader}>
                        <View style={[styles.mobileIconContainer, getIconContainerStyle(step.color)]}>
                          {/* Render the pre-built icon element */}
                          {step.iconElement}
                        </View>

                        <View>
                          <Text style={styles.mobileStepNumber}>STEP {step.id.toString()}</Text>
                          <Text style={styles.mobileStepTitle}>{step.title}</Text>
                        </View>
                      </View>

                      <Text style={styles.mobileStepDescription}>{step.description}</Text>
                    </View>

                    {activeStep < steps.length && (
                      <Pressable style={styles.nextButton} onPress={() => setActiveStep(activeStep + 1)}>
...
const styles = StyleSheet.create({
  ...
  connectionLine: {
    ...
    ...(Platform.OS === 'web'
      ? {
          backgroundImage:
            'linear-gradient(90deg, rgba(153, 69, 255, 0.8), rgba(68, 176, 255, 0.8), rgba(20, 241, 149, 0.8), rgba(153, 69, 255, 0.8))',
          borderRadius: 2
        }
      : {})
  },
  ...
  progressBar: {
    ...
    ...(Platform.OS === 'web'
      ? {
          backgroundImage:
            'linear-gradient(90deg, rgba(153, 69, 255, 0.3), rgba(68, 176, 255, 0.3), rgba(20, 241, 149, 0.3))'
        }
      : {})
  },
  stepButton: {
